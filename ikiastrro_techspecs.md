...
- Database name: **`ikiastrro_betav1`** (renamed from `vedic_horo_gen` on 2026-08-30).
  Connection string in `SqlConnectionFactory.CreateDefault()`.
- **Schema is one consolidated baseline: `db/ikiastrro_betav1.sql`** — whole schema (20
  ...
  - Fresh machine: run `db/ikiastrro_betav1.sql`.
  - Existing `vedic_horo_gen` DB: run `db/00_rename_db_to_ikiastrro_betav1.sql` (renames in
    place, keeps all data).
  - The former numbered migrations `001…034` are frozen under `db/_archive/` for history —
    do not add new numbered migrations; edit the consolidated script (and bump `betav1`
    when it becomes a breaking baseline).
...
- Career & Money are merged in the Web layer only; `LifeArea` enum in Core still has them
  separate.
- The database name (`ikiastrro_betav1`) diverges from the code namespace (`Ikiastrro`) —
  intentional (the `betav1` suffix versions the schema baseline).
