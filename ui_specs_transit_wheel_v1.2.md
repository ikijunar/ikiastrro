...
# Transit Wheel UI Specification v1.2
...
The supplied SVG now exposes stable saturn, jupiter, ahu, and ketu groups. Date and dasha changes apply coordinate-delta transforms to those groups while preserving the SVG artwork and ring geometry.

### Pointer dragging

Saturn, Jupiter, Rahu, and Ketu SVG groups are pointer draggable. Pointer longitude is converted to sidereal longitude; the nearest movement date is solved using the body's current ephemeris speed, clamped to the 36-month window, and the component refreshes all linked bodies in the same dasha context.

### Drag rollback

Pointer dragging was removed from the v1.2 implementation because it caused browser lifecycle and shutdown issues. Date and dasha selector movement remain supported.

### UI navigation clarification

The home page is the saved-person selection screen. The transit wheel is opened only after selecting a person at /transit-wheel/{Id}; it is not the master or home page.

### D1 natal binding correction

The transit wheel now loads the selected saved person's D1 ChartResult and ChartKeyDetails rows. Natal sign and degree values in the comparison layer are no longer hardcoded to Ramakrishnan; the selected chart identity controls the data source.

### Natal ring binding fix

The yellow natal ring now removes the reference template's fixed planet and house labels and renders the selected saved person's D1 planet abbreviations and whole-sign house numbers from the stored Ascendant.

### Transit-layer revalidation

Moon, Mars, and Venus are now calculated from Swiss sidereal positions for the active date and displayed in the comparison table and outer SVG annotation. They remain display-only: no tails, movement windows, or drag solving are applied. Saturn, Jupiter, Rahu, and Ketu continue through Gochara snapshots and movement transforms.

### Home landing screen reference

The supplied UI_SVG_Templates/ScreenDesign/InitialScreen-IkiAstrro.png is the Home visual authority. Home presents saved-person selection and routes to /transit-wheel/{Id}; the transit wheel is not rendered as the landing screen.

### 2026-09-07 verified selector/data implementation

Supersedes the earlier deterministic-preview implementation note: selectors now use actual saved Maha/Antar period dates and retain exact period start times. The date input and SVG centre share the active instant (UTC internally, IST display). The selected saved D1 loads natal planets, Gulika/Maandi and Ascendant; all nine transit comparison values are calculated at the active instant. Date and dasha interactions, natal stability, saved-person binding and responsive square geometry passed local browser checks. Drag rollback remains in effect. Full visual acceptance remains open for outer-label crowding and mobile text; see docs/superpowers/plans/2026-09-07-ui-resume.md.
