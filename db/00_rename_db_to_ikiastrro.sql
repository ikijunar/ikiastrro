-- =====================================================================
-- One-time: rename the existing 'vedic_horo_gen' database to 'ikiastrro'
...
--
-- After this, the app's connection string already points at [ikiastrro]
...
-- db/ikiastrro.sql instead (it builds the whole schema + reference data).
-- =====================================================================
USE [master];
GO

IF DB_ID(N'ikiastrro') IS NOT NULL
BEGIN
    PRINT 'ikiastrro already exists - nothing to do.';
END
ELSE IF DB_ID(N'vedic_horo_gen') IS NULL
BEGIN
    PRINT 'Neither vedic_horo_gen nor ikiastrro exists - run db/ikiastrro.sql for a fresh build.';
END
ELSE
BEGIN
    ALTER DATABASE [vedic_horo_gen] SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    ALTER DATABASE [vedic_horo_gen] MODIFY NAME = [ikiastrro];
    ALTER DATABASE [ikiastrro] SET MULTI_USER;
    PRINT 'Renamed vedic_horo_gen -> ikiastrro.';
END
GO

SELECT name, state_desc, user_access_desc, create_date
FROM sys.databases
WHERE name IN (N'vedic_horo_gen', N'ikiastrro');
GO
