...
reflects: master @ 802e673 + branch feat/graha-dignity-rule-layer (migrations 22–29, unpushed)
---
...
| 5 | Special Lagnas (45) — Bhaava, Hora, Ghati, Sree | migrations 28–29: `tbl_Dim_SpecialLagnas` (4, + usage/varga columns), `tbl_Rule_SpecialLagnaTimeRate` (BL/HL/GL), `tbl_Rule_SpecialLagnaFraction` (SL); taxonomy addendum `SPT_BL/HL/GL/SL` + calc-vocabulary concepts (sa/en). Engines: `HoraLagnaCalculator` ✓ only; Bhaava / Ghati / Sree **not built** | partial (DB layer added) | DB rule layer seeded vs §5.2–5.7 + §5.6 usage (`LifeAreaFocus` / `UsageContext` / `HouseReferenceScope` / `DasaLinkage` / `RelatedVargaChartId`). **Varga correlation (§6.3, soft):** HL↔D2 (Hora), GL↔D10 (Dasamsa); SL feeds Sudasa (rasi, no varga); BL none — special lagnas are *reference points* projected into every varga, not charts (§7.1), so the pairing is interpretive not a ch-5 rule. **Divergence:** BL `DegreesPerMinute` = 0.25 (§5.2 stated rate / classical `ishtakāla ÷ 5` / JHora); §5.2's method step + Example 7 imply 1.0 — book erratum, recorded in the BL row narrative (`UsedInBook = 0`). **Next:** build BhaavaLagna / GhatiLagna / SreeLagna engines off the rule rows; fold the taxonomy addendum into `TerminologySeed.cs`. |
