...
- `ikiastrro/docs/star-schema-rules-engine.md` (the `tbl_Rule_*` / `tbl_Rule_Sets` versioning dimension item 1 completes)
- `ikiastrro/docs/codex-review-loop.md` (how the review was produced)
...
- **Concurrent editor.** The `avastha-baaladi-jagradadi-star` session has been editing `db/ikiastrro.sql`, `tbl_Fact_PlanetAvastha`, and `vw_Chart_Consolidated` within the last hour. **Mitigation:** confirm it is paused or its work merged before Phase 1 writes; re-check `git status` + `db/*.sql` mtimes at the start of every phase; the `tbl_Fact_PlanetAvastha` `*Id` adds (§5.3) and its Phase-3 column drops are done in coordination with that session.
